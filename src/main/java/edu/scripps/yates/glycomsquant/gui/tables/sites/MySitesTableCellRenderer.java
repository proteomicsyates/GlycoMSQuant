...
class MySitesTableCellRenderer extends DefaultTableCellRenderer {
	...
	private static Logger log = Logger.getLogger(MySitesTableCellRenderer.class);

	@Override
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			int row, int column) {
		int columnIndex = -1;
		ColumnsSitesTable selectedColumn = null;
		try {
			for (final ColumnsSitesTable tableColumn : ColumnsSitesTable.values()) {
	...
	private String getToolTip(String value, ColumnsSitesTable column) {
	...
	private Color getColor(int row, ColumnsSitesTable selectedColumn) {
		Color color = null;
