	...
	public List<Object> getRunInfoList(File runPath, List<ColumnsRunTable> list, int index) {
		...
		for (final ColumnsRunTable column : list) {
			...
			case INPUT_DATA_FILE:
				ret.add(cleanString(String.valueOf(FilenameUtils.getName(
						ResultsProperties.getResultsProperties(runPath).getInputDataFile().getAbsolutePath()))));
				break;
			case NORMALIZE_REPLICATES:
				ret.add(ResultsProperties.getResultsProperties(runPath).getNormalizeReplicates());
				break;
			case NAME:
				ret.add(cleanString(ResultsProperties.getResultsProperties(runPath).getName()));
				break;
			case RUN_PATH:
				ret.add(cleanString(FilenameUtils.getName(runPath.getAbsolutePath())));
				break;

			case THRESHOLD:
				ret.add(ResultsProperties.getResultsProperties(runPath).getIntensityThreshold());
				break;
			case CALCULATE_PEPTIDE_PROPORTIONS_FIRST:
				ret.add(ResultsProperties.getResultsProperties(runPath).getCalculatePeptideProportionsFirst());
				break;
			default:
